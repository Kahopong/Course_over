      ...
      async (req, username, password, done) => {
        ...
        try {
          ...
          if (users.length !== 0) {
            console.log("test1");
            return done(null, false, {
              message: "This Account is already being used!",
            });
          }
          ...
          let userloginId = await knex("user_login")
            .insert(newUserlogin)
            .returning("id");
          newUserlogin.id = userloginId[0];

          done(null, newUserlogin, {
            message: `Hey ${newUserlogin.username}! You can now login to use the death note!`,
          });
        } catch (err) {
          if (err) {
            done(err);
    ...
    new LocalStrategy(
      ...
      async (req, username, password, done) => {
        ...
        try {
          ...
          let userloginId = await knex("shop_login")
            .insert(newUserlogin)
            .returning("id");
          newUserlogin.id = userloginId[0];
